...
from espnet2.asr.roberta_joint import RobertaJointModel
...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.did_tokens = [self.token_list.index(token) for token in self.token_list_did]
        ...
        self.enc_roberta = RobertaJointModel(ctc, token_list, len(self.did_tokens), self.did_tokens, tokenizer_path)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.ctc_weight != 0.0:
            ####################################AV####################################
            enc_rob_out = self.enc_roberta(encoder_out, encoder_out_lens)
    
            # get the labels
            label = torch.tensor([int(self.token_list[j])-1 for j in text[:,:1]], device=encoder_out.device)

            loss_ce, cer_ce = self._calc_ce_loss(enc_rob_out, label)
    ...
    def _calc_ce_loss(
        ...
        if not self.training and self.error_calculator is not None:
            ...
            cer_ce = self.error_calculator(ys_hat.unsqueeze(0).cpu(), ys_pad.unsqueeze(0).cpu(), is_ctc=True)
        return loss_ce, cer_ce
    ####################################AV####################################
